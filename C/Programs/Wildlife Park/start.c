...
void display_results(int package_time, int package_kind, int extra_things, float total_cost, int number_of_tickets, int day)
{
	...
	srand ( time(NULL) );
	// ticket_number = rand();
	ticket_number = (rand() % (10000 - 0 + 1)) + 0;
	...
	getch();
	//--------------------------------------------------------------------------------
	store_ticket_data(ticket_number, package_kind, extra_things, day);
	run_again();
}
...
void inital_choice()
{
	int initial_user_choice;
	print_slowly("\tChoose:\n\t\t1) Book a New Ticket\n\t\t2) Check Booked Ticket\n\t\t3) Exit\n\t>");
	scanf("%d", &initial_user_choice);
	if(initial_user_choice==1)
		display_info('0', 0);
	else if(initial_user_choice==2)
		read_ticket_data();
	else if(initial_user_choice==3)
		exit(0);
	else
	{
		system("cls");
